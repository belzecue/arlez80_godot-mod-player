...
func play( from_position:float = 0.0 ):
	...
	if from_position == 0.0:
		...
		self.next_row_remain_second = 0.0
...
func _process( delta:float ):
	if self.mod_data != null:
		if self.playing:
			...
			while self.next_tick_remain_second <= 0.0 and self.processed_tick_count < self.tick_per_row:
				self._process_tick( )
...
func _process_row_for_channel( channel:GodotModPlayerChannelStatus, note:Mod.ModPatternNote ) -> void:
	...
	if note.note == 96:
		...
		var inst:GodotModPlayerInstrument = self.instruments[( note.instrument - 1 ) % self.instruments.size( )]
...
func _process_tick_for_channel( channel:GodotModPlayerChannelStatus, note:Mod.ModPatternNote, disable_channel_row:bool ) -> void:
	...
	match note.effect_command:
		...
		0x0D:	# Pattern break
			if disable_channel_row:
				self.pattern_position_jump_point = self.pattern_position + 1
		...
		0x0E:	# 拡張コマンド
			match ( note.effect_param >> 4 ):
				...
				0x04:	# Vibrato type
					...
					channel.vibrato.update( )
				0x06:	# Pattern loop
					printerr( "not implemented: E6y Pattern Loop" )
				0x07:	# Tremoro type
				...
				0x0C:	# Note cut
					if self.processed_tick_count - 1 == note.volume & 0x0F:
						channel.volume = 0
				0x0D:	# Note delay
					printerr( "not implemented: EDy Note Delay" )
				0x0E:	# Pattern delay
					self.extra_tick = ( note.effect_param & 0x0F )
					printerr( "not implemented: EEy Pattern Delay" )
				_:
					printerr( "unknown extended command: %02x : %04x" % [ note.effect_command, note.effect_param ] )
		0x0F:	# Tick / Tempo
